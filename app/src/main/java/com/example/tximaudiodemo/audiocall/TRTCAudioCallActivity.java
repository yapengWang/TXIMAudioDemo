...
//            ToastUtils.showLong(getString(R.string.trtcaudiocall_toast_call_error_msg, code, msg));
        ...
        public void onUserVoiceVolume(Map<String, Integer> volumeMap) {
            for (Map.Entry<String, Integer> entry : volumeMap.entrySet()) {
                ...
                Log.e(TAG, "userId -> " + userId + "Volume ->" + entry.getValue());
                TRTCAudioLayout layout = mLayoutManagerTRTC.findAudioCallLayout(userId);
                if (layout != null) {
...
//        mRingVibrateHelper =   TimRingVibrateHelper.getInstance();
        initView();
        initData();
        initListener();
    }
    ...
    public void showWaitingResponseView() {
        //响铃或者震动
//        mRingVibrateHelper.initRemoteCallRinging();
        //1. 展示对方的画面
        ...
        Glide.with(this).load(mSponsorUserModel.userAvatar).into(layout.getImageView());
        //2. 展示电话对应界面
...
//            mRingVibrateHelper.initLocalCallRinging();
//        }
        //1. 展示自己的界面
        mLayoutManagerTRTC.setMySelfUserId(mSelfModel.userId);
        addUserToManager(mSelfModel);
...
//        }
    ...
    private void showOtherInvitingUserView() {
        ...
        for (int index = 0; index < mOtherInvitingUserModelList.size() && index < MAX_SHOW_INVITING_USER; index++) {
            ...
            Glide.with(this).load(userModel.userAvatar).into(imageView);
    ...
    private TRTCAudioLayout addUserToManager(UserModel userModel) {
        ...
        Glide.with(this).load(userModel.userAvatar).into(layout.getImageView());
        return layout;
    }
